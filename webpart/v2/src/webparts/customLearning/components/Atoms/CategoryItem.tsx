...
export default class CategoryItem extends React.Component<ICategoryItemProps, ICategoryItemState> {
  ...
  public render(): React.ReactElement<ICategoryItemProps> {
    try {
      ...
      return (
        <div className="category-item" onClick={() => this.props.selectItem(Templates.SubCategory, this.props.subcategoryId)} >
          <div className="category-icon">
            <img src={categoryImage} />
          </div>
          <div className="category-label">{this.props.subcategoryName}</div>
        </div>
      );
    } catch (err) {
